...
import { Printer, UploadCloud, File, AlertCircle, CheckCircle2, ChevronRight, Hash, X } from 'lucide-react'
  ...
  async function handleUpload(event: React.FormEvent) {
    ...
    try {
      ...
      if (file.size > device.max_file_size_mb * 1024 * 1024) {
        throw new Error(`Ukuran file terlalu besar. Maksimal ${device.max_file_size_mb}MB.`)
      }

      if (!device.accepted_mime_types.includes(file.type)) {
        throw new Error('Tipe/format file ini tidak didukung oleh printer.')
      }
      ...
      setSuccess(true)
      toast.success('File berhasil dikirim ke printer!')
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div 
        ...
        className="w-full max-w-lg bg-white/80 backdrop-blur-xl border border-white rounded-[2rem] shadow-2xl shadow-slate-200/50 relative z-10 overflow-hidden"
      >
            ...
            {success ? (
              ...
              >
                ...
                <p className="text-slate-500 mb-8 max-w-[260px] mx-auto">
                  File Anda telah masuk ke antrean. Silakan konfirmasi pembayaran ke operator.
                </p>
                <button
                  onClick={resetForm}
              ...
              >
                  ...
                  <div 
                    ...
                    className={`relative border-2 border-dashed rounded-3xl p-8 text-center cursor-pointer transition-all ${
                      file 
                        ...
                        : 'border-slate-300 hover:border-indigo-400 hover:bg-slate-50 bg-slate-50/50'
                    }`}
                  >
                    {file ? (
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-xs text-slate-500 mt-2 max-w-[200px] mx-auto">
                          Maksimal {device?.max_file_size_mb}MB. Didukung: PDF, Word, Excel, Gambar.
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Jumlah</label>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div 
                      ...
                      className={`cursor-pointer border rounded-xl p-3 text-center transition-all ${colorMode === 'bw' ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white/50 text-slate-600 hover:border-slate-300'}`}
                    >
                      <span className="text-sm font-medium">Hitam Putih</span>
                    </div>
                    <div 
                      onClick={() => setColorMode('color')}
                      className={`cursor-pointer border rounded-xl p-3 text-center transition-all ${colorMode === 'color' ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-200 bg-white/50 text-slate-600 hover:border-slate-300'}`}
                    >
                      <span className="text-sm font-medium">Berwarna</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    className="w-full rounded-xl border border-slate-200 bg-white/50 px-4 py-3 text-sm outline-none transition-all focus:ring-2 focus:ring-indigo-500/50 resize-none h-24"
                    value={note}
                    onChange={(event) => setNote(event.target.value)}
                    placeholder="Contoh: Tolong print bolak-balik ya..."
                  />
                </div>

                <button
                  disabled={uploading || !device?.is_accepting_jobs || !file}
                  className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white py-4 font-bold text-sm transition-all shadow-xl shadow-indigo-600/20 disabled:opacity-50 disabled:shadow-none flex justify-center items-center gap-2 group"
                >
                  {uploading ? (
                    <>
